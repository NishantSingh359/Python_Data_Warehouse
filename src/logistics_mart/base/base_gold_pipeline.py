...
class BaseGoldPipeline(ABC):
    ...
    def run(self):
        ...
        try:
            ...
            logging.exception(
                f"{self.layer} | {self.mart} | {self.domain} | {self.table} | "
    ...
    def log(self, step, msg=""):
        logging.info(
            f"{self.layer} | {self.mart} | {self.domain} | {step} | {self.table} {msg}"
        )

    def log_shape(self, df):
        logging.info(
            f"{self.layer} | {self.mart} | {self.domain} | SHAPE | {self.table} | "
    ...
    def log_time(self, start):
        ...
        logging.info(
            f"{self.layer} | {self.mart} | {self.domain} | TIME  | {self.table} | duration_sec={duration}"
        )
