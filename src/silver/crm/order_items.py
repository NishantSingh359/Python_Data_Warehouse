...
class Order_itemsSilver(BaseSilverPipeline):
    ...
    def clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        order_item_id = df['order_item_id'].str.replace(r'\D','', regex=True).replace({'':np.nan,'nan':np.nan})
        ...
        order_id =      df['order_id'].replace(r'\D','',regex=True).replace({'':np.nan,'nan':np.nan})
        ...
        item_id =       df['item_id'].str.strip().replace(r'\D','',regex=True).replace({'':np.nan,'nan':np.nan})
        ...
        unit_price =  pd.to_numeric(df['unit_price'].str.strip().str.replace('-','').replace({'nan':np.nan}), errors= 'coerce')
        df['unit_price'] =  unit_price.where((unit_price > 0) & (unit_price < 1000), np.nan)
        ...
        # Fill NaN item_id 
        merg =    df.merge(unique_price_menu, left_on='unit_price', right_on='selling_price', how='left', suffixes=('', '_menu'))
        item_id['item_id'] = merg['item_id'].fillna(merg['item_id_menu'])
        item_id = item_id['item_id']

        quantity =   pd.to_numeric(df['quantity'].str.strip().str.replace('-','').replace({'nan':np.nan}), errors= 'coerce').astype('Int32')
        quantity =   quantity.where((quantity > 0) & (quantity < 50), np.nan)
        merg =       df.merge(menu[['item_id', 'selling_price']], on='item_id', how='left')
        unit_price = merg['unit_price'].fillna(merg['selling_price'])
        line_total = pd.to_numeric(df['line_total'].str.strip().str.replace('-','').replace({'nan':np.nan}), errors= 'coerce')
