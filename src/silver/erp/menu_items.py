...
class Menu_itemsSilver(BaseSilverPipeline):
    ...
    def clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        item_id =    item_id.fillna(df['item_name'].replace(r'\D', '', regex=True).replace({'':np.nan}))
        ...
        category =   df['category'].str.strip().replace({'nan':np.nan}).str.lower()
        cuisine =    df['cuisine'].str.strip().replace({'nan':np.nan}).str.lower()

        selling_price = df['selling_price'].astype(float)
